...
const COMPONENTS_TO_VIEW = [
    "HStackComponent", "VStackComponent", "HStack", "Stack", "VStack", "Center", "Box"
];
...
function processFile(filePath) {
    ...
    body.forEach((node) => {
        if (
            ...
            let viewImported = node.specifiers.some(
                (spec) =>
                    spec.type === "ImportSpecifier" && spec.imported.name === "View"
            );
        ...
        if (
            node.type === "ImportDeclaration" &&
            node.source.value === "~/components/ui/text"
        ) {
    ...
    if (!hasNewTextImport && hasTextReplacement) {
        body.unshift(
            b.importDeclaration(
                [b.importDefaultSpecifier(b.identifier("Text"))],
                b.literal("~/components/ui/text")
            )
        );
    }
...
(async () => {
    try {
        ...
        files.map(file => path.join(targetRoot, file))
