...
export const generateExcelFile = async (data: any[], fileName: string = 'Untitled file') => {
    ...
    const filePath = `${RNBlobUtil.fs.dirs.DocumentDir}/${fileName}.xlsx`;

    RNBlobUtil.fs.writeFile(filePath, wbout, 'ascii')
        .then((r: any) => {})
        .catch((e: any) => {
...
export const writeToStorage = async (generatedFile: any, fileName: string) => {
    ...
    RNBlobUtil.fs.writeFile(filePath, generatedFile, 'ascii')
        .then((r: any) => {})
        .catch((e: any) => {
            Alert.alert('Unable to generate file', e);
        });
