...
export const DepartmentReportListRow: React.FC<IReportProps> = props => {
    const handlePress = () => {
        router.push({
            pathname: `/reports/${ReportRouteIndex[props?.departmentName]}` as any,
            params: NESTED_PARAM_DEPTS.has(props?.departmentName) ? { data: JSON.stringify(props) } : (props as any),
        });
    };
