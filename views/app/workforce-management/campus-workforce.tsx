    ...
    return (
        <ErrorBoundary>
            ...
            <ViewWrapper scroll onRefresh={refresh} refreshing={isRefreshing}>
                {campusInfo.map((item, index) =>
                    isLoading ? (
                        ...
                        <View key={index} className="my-12 px-6 justify-start">
                            <Text>{item.name}</Text>
                            <Text className="font-bold ml-10">{item.value}</Text>
                        </View>
                    )
                )}
                    ...
                    <View className="my-16 p-8 flex-1 justify-evenly w-100% border-0.4 rounded-8">
                        {summaryList.map((item, index) => (
                            ...
                            >
                                ...
                                <Text className={cn('text-4xl', item.color)}>{item.value || 0}</Text>
                            </View>
                        ))}
                    </View>
                )}
                ...
                <View>
                    <View className="py-3 mb-2 flex-row flex-1 flex-wrap">
                        {isLoading ? (
