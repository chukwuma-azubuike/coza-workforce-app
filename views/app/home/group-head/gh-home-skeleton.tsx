...
const ClockCardSkeleton: React.FC = () => (
    <Card className="min-h-[190px]">
        <View className="p-4 gap-4">
            {/* Header label: "TAP TO CLOCK IN" */}
            <View className="flex-row items-center gap-2">
                <Skeleton className="w-4 h-4 rounded-full" />
                <Skeleton className="h-3.5 w-36 rounded" />
            </View>

            {/* Status message + service name */}
...
// ─── Reports status skeleton ──────────────────────────────────────────────────
const ReportsStatusSkeleton: React.FC = () => (
    <View className="gap-3">
        ...
        <View className="gap-2">
            {[1, 2, 3].map(i => (
                <View
                    key={i}
                    className="flex-row items-center justify-between rounded-xl px-4 py-3 bg-muted-background"
                    style={{ height: 48 }}
                >
                    <Skeleton className="h-4 w-2/5 rounded" />
...
// ─── Composed full-screen skeleton ────────────────────────────────────────────
const GHHomeSkeleton: React.FC = () => (
